...
interface Props {
  /** Visual size in px. */
  size?: number;
  /** Color of the brand mark glyph. */
  color?: string;
  /** Tinted background container color. Pass null for transparent. */
  background?: string | null;
  /** When true, applies a subtle pulse + glow animation. */
  animated?: boolean;
}

/**
 * The 5th Element brand "5" mark, extracted from the production /app/icon.svg.
 * Used in the AI hero variants as the avatar/badge for the "AI agent" element.
 */
export default function FifthElementMark({
  size = 36,
  color = '#00BE9D',
  ...
  return (
    <div
      className={`flex items-center justify-center flex-shrink-0 relative ${animated ? 'fe-mark-anim' : ''}`}
      style={{
        width: size,
        height: size,
        borderRadius: size * 0.22,
        background: background ?? 'transparent',
        border: background ? `1px solid ${color}33` : 'none',
      }}
      aria-hidden
    >
      {animated && (
        <span
          className="absolute inset-0 rounded-[inherit] fe-mark-glow"
          style={{ boxShadow: `0 0 ${size * 0.6}px ${color}55, 0 0 ${size}px ${color}33` }}
        />
      )}
      <svg
